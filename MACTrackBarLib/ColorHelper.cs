...
internal static class ColorHelper
{
    ...
    {
        var baseAsFloat = (float)@base / 255;
        var blendAsFloat = (float)blend / 255;
        if (blendAsFloat < 0.5)
        {
            return (int)((2 * baseAsFloat * blendAsFloat + Math.Pow(baseAsFloat, 2) * (1 - 2 * blendAsFloat)) * 255);
        }
        return (int)((Math.Sqrt(baseAsFloat) * (2 * blendAsFloat - 1) + 2 * baseAsFloat * (1 - blendAsFloat)) * 255);
    }
    ...
    /// <param name="blend"></param>
    /// <returns></returns>
    private static int OverlayMath(int @base, int blend)
    {
        var baseAsDouble = (double)@base / 255;
        var blendAsDouble = (double)blend / 255;
        if (baseAsDouble < 0.5)
        {
            return (int)(2 * baseAsDouble * blendAsDouble * 255);
        }
        return (int)((1 - 2 * (1 - baseAsDouble) * (1 - blendAsDouble)) * 255);
    }
}
